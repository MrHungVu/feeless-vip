import {
  ...
  hexToNumber,
  numberToHex
} from "./chunk-RL2W6EJD.js";
import {
  keccak_256
} from "./chunk-4BZL3DYM.js";
import {
  require_elliptic
} from "./chunk-LNSV4GTR.js";
import {
  HashMD
} from "./chunk-CDAGBNQA.js";
import {
  rotl,
  wrapConstructor
} from "./chunk-FADKKGIW.js";
import {
  IEvents,
  bases,
  esm_exports,
...
var k = (e2, s2, r2) => s2 in e2 ? T(e2, s2, { enumerable: true, configurable: true, writable: true, value: r2 }) : e2[s2] = r2;
var i2 = (e2, s2, r2) => k(e2, typeof s2 != "symbol" ? s2 + "" : s2, r2);
var J = class {
  constructor(s2) {
...
var shiftsL = idxL.map((idx, i4) => idx.map((j) => shifts[i4][j]));
var shiftsR = idxR.map((idx, i4) => idx.map((j) => shifts[i4][j]));
...
// ../node_modules/@walletconnect/solana-adapter/node_modules/uint8arrays/esm/src/alloc.js
...
// ../node_modules/@walletconnect/solana-adapter/node_modules/uint8arrays/esm/src/concat.js
...
// ../node_modules/@walletconnect/solana-adapter/node_modules/uint8arrays/esm/src/util/bases.js
...
// ../node_modules/@walletconnect/solana-adapter/node_modules/uint8arrays/esm/src/from-string.js
...
// ../node_modules/@walletconnect/solana-adapter/node_modules/uint8arrays/esm/src/to-string.js
...
function je(e2, ...t2) {
  if (!Io(e2)) throw new Error("Uint8Array expected");
...
function un(e2) {
  for (let t2 = 0; t2 < e2.length; t2++) e2[t2] = $o(e2[t2]);
}
function Ro(e2) {
...
function Se(e2 = 32) {
  if (be && typeof be.getRandomValues == "function") return be.getRandomValues(new Uint8Array(e2));
  if (be && typeof be.randomBytes == "function") return be.randomBytes(e2);
...
for (let e2 = 0, t2 = Ue, n2 = 1, r2 = 0; e2 < 24; e2++) {
  ...
  let o2 = Po;
  for (let s2 = 0; s2 < 7; s2++) t2 = (t2 << Ue ^ (t2 >> Lo) * Co) % jo, t2 & Bo && (o2 ^= Ue << (Ue << BigInt(s2)) - Ue);
  hn.push(o2);
}
...
function Mo(e2, t2 = 24) {
  ...
  for (let r2 = 24 - t2; r2 < 24; r2++) {
    ...
    for (let i4 = 0; i4 < 10; i4 += 2) {
      const c3 = (i4 + 8) % 10, u3 = (i4 + 2) % 10, a2 = n2[u3], l3 = n2[u3 + 1], f6 = pn(a2, l3, 1) ^ n2[c3], d2 = gn(a2, l3, 1) ^ n2[c3 + 1];
      for (let g = 0; g < 50; g += 10) e2[i4 + g] ^= f6, e2[i4 + g + 1] ^= d2;
    }
    let o2 = e2[2], s2 = e2[3];
    for (let i4 = 0; i4 < 24; i4++) {
      const c3 = dn[i4], u3 = pn(o2, s2, c3), a2 = gn(o2, s2, c3), l3 = ln[i4];
...
var Lt = class _Lt extends it {
  constructor(t2, n2, r2, o2 = false, s2 = 24) {
    if (super(), this.blockLen = t2, this.suffix = n2, this.outputLen = r2, this.enableXOF = o2, this.rounds = s2, this.pos = 0, this.posOut = 0, this.finished = false, this.destroyed = false, Ne(r2), 0 >= this.blockLen || this.blockLen >= 200) throw new Error("Sha3 supports only keccak-f1600 function");
...
function F(e2, ...t2) {
  if (!Bn(e2)) throw new Error("Uint8Array expected");
  if (t2.length > 0 && !t2.includes(e2.length)) throw new Error("Uint8Array expected of length " + t2 + ", got length=" + e2.length);
}
...
var hs = (e2, t2) => {
  function n2(r2, ...o2) {
    ...
    return { encrypt(l3, f6) {
      ...
      return u3 = true, F(l3), c3(i4.encrypt.length, f6), i4.encrypt(l3, f6);
    }, decrypt(l3, f6) {
      if (F(l3), s2 && l3.length < s2) throw new Error("invalid ciphertext length: smaller than tagLength=" + s2);
      return c3(i4.decrypt.length, f6), i4.decrypt(l3, f6);
    } };
  }
...
function Cn(e2, t2, n2 = true) {
  ...
  if (n2 && !ps(t2)) throw new Error("invalid output, must be aligned");
  return t2;
}
...
function Es(e2, t2, n2, r2, o2, s2, i4, c3) {
  const u3 = o2.length, a2 = new Uint8Array(Ve), l3 = se(a2), f6 = yt(o2) && yt(s2), d2 = f6 ? se(o2) : Vn, g = f6 ? se(s2) : Vn;
  for (let y4 = 0; y4 < u3; i4++) {
    if (e2(t2, n2, r2, l3, i4, c3), i4 >= Mn) throw new Error("arx: counter overflow");
    const h4 = Math.min(Ve, u3 - y4);
    if (f6 && h4 === Ve) {
      const m2 = y4 / 4;
      if (y4 % 4 !== 0) throw new Error("arx: invalid block position");
      for (let L = 0, b2; L < ws; L++) b2 = m2 + L, g[b2] = d2[b2] ^ l3[L];
      y4 += Ve;
      continue;
    }
    for (let m2 = 0, L; m2 < h4; m2++) L = y4 + m2, s2[L] = o2[L] ^ a2[m2];
    y4 += h4;
  }
}
function vs(e2, t2) {
  ...
  return ht(o2), ht(i4), jn(s2), jn(n2), (c3, u3, a2, l3, f6 = 0) => {
    F(c3), F(u3), F(a2);
    const d2 = a2.length;
    if (l3 === void 0 && (l3 = new Uint8Array(d2)), F(l3), ht(f6), f6 < 0 || f6 >= Mn) throw new Error("arx: counter overflow");
    if (l3.length < d2) throw new Error(`arx: output (${l3.length}) is shorter than data (${d2})`);
    const g = [];
    let y4 = c3.length, h4, m2;
    if (y4 === 32) g.push(h4 = gt(c3)), m2 = bs;
    else if (y4 === 16 && n2) h4 = new Uint8Array(32), h4.set(c3), h4.set(c3, 16), m2 = ms, g.push(h4);
    ...
    const b2 = 16 - o2;
    if (b2 !== u3.length) throw new Error(`arx: nonce must be ${b2} or 16 bytes`);
    if (b2 !== 12) {
      const O3 = new Uint8Array(12);
      O3.set(u3, s2 ? 0 : 12 - u3.length), u3 = O3, g.push(u3);
    }
    const _2 = se(u3);
    return Es(e2, m2, L, _2, a2, l3, f6, i4), Ee(...g), l3;
  };
}
...
var xs = class {
  constructor(t2) {
    ...
    const n2 = M2(t2, 0), r2 = M2(t2, 2), o2 = M2(t2, 4), s2 = M2(t2, 6), i4 = M2(t2, 8), c3 = M2(t2, 10), u3 = M2(t2, 12), a2 = M2(t2, 14);
    ...
    for (let l3 = 0; l3 < 8; l3++) this.pad[l3] = M2(t2, 16 + 2 * l3);
  }
  process(t2, n2, r2 = false) {
    const o2 = r2 ? 0 : 2048, { h: s2, r: i4 } = this, c3 = i4[0], u3 = i4[1], a2 = i4[2], l3 = i4[3], f6 = i4[4], d2 = i4[5], g = i4[6], y4 = i4[7], h4 = i4[8], m2 = i4[9], L = M2(t2, n2 + 0), b2 = M2(t2, n2 + 2), _2 = M2(t2, n2 + 4), O3 = M2(t2, n2 + 6), k3 = M2(t2, n2 + 8), E2 = M2(t2, n2 + 10), B2 = M2(t2, n2 + 12), j = M2(t2, n2 + 14);
    let v2 = s2[0] + (L & 8191), I2 = s2[1] + ((L >>> 13 | b2 << 3) & 8191), w = s2[2] + ((b2 >>> 10 | _2 << 6) & 8191), R2 = s2[3] + ((_2 >>> 7 | O3 << 9) & 8191), A2 = s2[4] + ((O3 >>> 4 | k3 << 12) & 8191), T2 = s2[5] + (k3 >>> 1 & 8191), N = s2[6] + ((k3 >>> 14 | E2 << 2) & 8191), S2 = s2[7] + ((E2 >>> 11 | B2 << 5) & 8191), U2 = s2[8] + ((B2 >>> 8 | j << 8) & 8191), $ = s2[9] + (j >>> 5 | o2), p2 = 0, C4 = p2 + v2 * c3 + I2 * (5 * m2) + w * (5 * h4) + R2 * (5 * y4) + A2 * (5 * g);
    p2 = C4 >>> 13, C4 &= 8191, C4 += T2 * (5 * d2) + N * (5 * f6) + S2 * (5 * l3) + U2 * (5 * a2) + $ * (5 * u3), p2 += C4 >>> 13, C4 &= 8191;
    let D = p2 + v2 * u3 + I2 * c3 + w * (5 * m2) + R2 * (5 * h4) + A2 * (5 * y4);
    p2 = D >>> 13, D &= 8191, D += T2 * (5 * g) + N * (5 * d2) + S2 * (5 * f6) + U2 * (5 * l3) + $ * (5 * a2), p2 += D >>> 13, D &= 8191;
    let P3 = p2 + v2 * a2 + I2 * u3 + w * c3 + R2 * (5 * m2) + A2 * (5 * h4);
    p2 = P3 >>> 13, P3 &= 8191, P3 += T2 * (5 * y4) + N * (5 * g) + S2 * (5 * d2) + U2 * (5 * f6) + $ * (5 * l3), p2 += P3 >>> 13, P3 &= 8191;
    let G2 = p2 + v2 * l3 + I2 * a2 + w * u3 + R2 * c3 + A2 * (5 * m2);
    p2 = G2 >>> 13, G2 &= 8191, G2 += T2 * (5 * h4) + N * (5 * y4) + S2 * (5 * g) + U2 * (5 * d2) + $ * (5 * f6), p2 += G2 >>> 13, G2 &= 8191;
    let X = p2 + v2 * f6 + I2 * l3 + w * a2 + R2 * u3 + A2 * c3;
    p2 = X >>> 13, X &= 8191, X += T2 * (5 * m2) + N * (5 * h4) + S2 * (5 * y4) + U2 * (5 * g) + $ * (5 * d2), p2 += X >>> 13, X &= 8191;
    let Z2 = p2 + v2 * d2 + I2 * f6 + w * l3 + R2 * a2 + A2 * u3;
    p2 = Z2 >>> 13, Z2 &= 8191, Z2 += T2 * c3 + N * (5 * m2) + S2 * (5 * h4) + U2 * (5 * y4) + $ * (5 * g), p2 += Z2 >>> 13, Z2 &= 8191;
    let he2 = p2 + v2 * g + I2 * d2 + w * f6 + R2 * l3 + A2 * a2;
    p2 = he2 >>> 13, he2 &= 8191, he2 += T2 * u3 + N * c3 + S2 * (5 * m2) + U2 * (5 * h4) + $ * (5 * y4), p2 += he2 >>> 13, he2 &= 8191;
    let pe = p2 + v2 * y4 + I2 * g + w * d2 + R2 * f6 + A2 * l3;
    p2 = pe >>> 13, pe &= 8191, pe += T2 * a2 + N * u3 + S2 * c3 + U2 * (5 * m2) + $ * (5 * h4), p2 += pe >>> 13, pe &= 8191;
    let ge = p2 + v2 * h4 + I2 * y4 + w * g + R2 * d2 + A2 * f6;
    p2 = ge >>> 13, ge &= 8191, ge += T2 * l3 + N * a2 + S2 * u3 + U2 * c3 + $ * (5 * m2), p2 += ge >>> 13, ge &= 8191;
    let ye = p2 + v2 * m2 + I2 * h4 + w * y4 + R2 * g + A2 * d2;
    p2 = ye >>> 13, ye &= 8191, ye += T2 * f6 + N * l3 + S2 * a2 + U2 * u3 + $ * c3, p2 += ye >>> 13, ye &= 8191, p2 = (p2 << 2) + p2 | 0, p2 = p2 + C4 | 0, C4 = p2 & 8191, p2 = p2 >>> 13, D += p2, s2[0] = C4, s2[1] = D, s2[2] = P3, s2[3] = G2, s2[4] = X, s2[5] = Z2, s2[6] = he2, s2[7] = pe, s2[8] = ge, s2[9] = ye;
  }
  finalize() {
...
function As(e2, t2, n2, r2, o2, s2 = 20) {
  let i4 = e2[0], c3 = e2[1], u3 = e2[2], a2 = e2[3], l3 = t2[0], f6 = t2[1], d2 = t2[2], g = t2[3], y4 = t2[4], h4 = t2[5], m2 = t2[6], L = t2[7], b2 = o2, _2 = n2[0], O3 = n2[1], k3 = n2[2], E2 = i4, B2 = c3, j = u3, v2 = a2, I2 = l3, w = f6, R2 = d2, A2 = g, T2 = y4, N = h4, S2 = m2, U2 = L, $ = b2, p2 = _2, C4 = O3, D = k3;
  for (let G2 = 0; G2 < s2; G2 += 2) E2 = E2 + I2 | 0, $ = x($ ^ E2, 16), T2 = T2 + $ | 0, I2 = x(I2 ^ T2, 12), E2 = E2 + I2 | 0, $ = x($ ^ E2, 8), T2 = T2 + $ | 0, I2 = x(I2 ^ T2, 7), B2 = B2 + w | 0, p2 = x(p2 ^ B2, 16), N = N + p2 | 0, w = x(w ^ N, 12), B2 = B2 + w | 0, p2 = x(p2 ^ B2, 8), N = N + p2 | 0, w = x(w ^ N, 7), j = j + R2 | 0, C4 = x(C4 ^ j, 16), S2 = S2 + C4 | 0, R2 = x(R2 ^ S2, 12), j = j + R2 | 0, C4 = x(C4 ^ j, 8), S2 = S2 + C4 | 0, R2 = x(R2 ^ S2, 7), v2 = v2 + A2 | 0, D = x(D ^ v2, 16), U2 = U2 + D | 0, A2 = x(A2 ^ U2, 12), v2 = v2 + A2 | 0, D = x(D ^ v2, 8), U2 = U2 + D | 0, A2 = x(A2 ^ U2, 7), E2 = E2 + w | 0, D = x(D ^ E2, 16), S2 = S2 + D | 0, w = x(w ^ S2, 12), E2 = E2 + w | 0, D = x(D ^ E2, 8), S2 = S2 + D | 0, w = x(w ^ S2, 7), B2 = B2 + R2 | 0, $ = x($ ^ B2, 16), U2 = U2 + $ | 0, R2 = x(R2 ^ U2, 12), B2 = B2 + R2 | 0, $ = x($ ^ B2, 8), U2 = U2 + $ | 0, R2 = x(R2 ^ U2, 7), j = j + A2 | 0, p2 = x(p2 ^ j, 16), T2 = T2 + p2 | 0, A2 = x(A2 ^ T2, 12), j = j + A2 | 0, p2 = x(p2 ^ j, 8), T2 = T2 + p2 | 0, A2 = x(A2 ^ T2, 7), v2 = v2 + I2 | 0, C4 = x(C4 ^ v2, 16), N = N + C4 | 0, I2 = x(I2 ^ N, 12), v2 = v2 + I2 | 0, C4 = x(C4 ^ v2, 8), N = N + C4 | 0, I2 = x(I2 ^ N, 7);
  let P3 = 0;
  r2[P3++] = i4 + E2 | 0, r2[P3++] = c3 + B2 | 0, r2[P3++] = u3 + j | 0, r2[P3++] = a2 + v2 | 0, r2[P3++] = l3 + I2 | 0, r2[P3++] = f6 + w | 0, r2[P3++] = d2 + R2 | 0, r2[P3++] = g + A2 | 0, r2[P3++] = y4 + T2 | 0, r2[P3++] = h4 + N | 0, r2[P3++] = m2 + S2 | 0, r2[P3++] = L + U2 | 0, r2[P3++] = b2 + $ | 0, r2[P3++] = _2 + p2 | 0, r2[P3++] = O3 + C4 | 0, r2[P3++] = k3 + D | 0;
}
...
function Kn(e2, t2, n2, r2, o2) {
  ...
  kn(u3, 0, BigInt(o2 ? o2.length : 0), true), kn(u3, 8, BigInt(r2.length), true), i4.update(c3);
  const a2 = i4.digest();
  return Ee(s2, c3), a2;
}
var _s = (e2) => (t2, n2, r2) => ({ encrypt(s2, i4) {
  const c3 = s2.length;
  i4 = Cn(c3 + 16, i4, false), i4.set(s2);
  const u3 = i4.subarray(0, -16);
  e2(t2, n2, u3, u3, 1);
  const a2 = Kn(e2, t2, n2, u3, r2);
  return i4.set(a2, c3), Ee(a2), i4;
}, decrypt(s2, i4) {
  i4 = Cn(s2.length - 16, i4, false);
  const c3 = s2.subarray(0, -16), u3 = s2.subarray(-16), a2 = Kn(e2, t2, n2, c3, r2);
...
var js = class extends it {
  ...
  digestInto(t2) {
    ...
    for (let f6 = i4; f6 < o2; f6++) n2[f6] = 0;
    ...
    for (let f6 = 0; f6 < a2; f6++) c3.setUint32(4 * f6, l3[f6], s2);
  }
  digest() {
    const { buffer: t2, outputLen: n2 } = this;
    this.digestInto(t2);
...
var ks = class extends js {
  ...
  process(t2, n2) {
    for (let f6 = 0; f6 < 16; f6++, n2 += 4) ce[f6] = t2.getUint32(n2, false);
    for (let f6 = 16; f6 < 64; f6++) {
      const d2 = ce[f6 - 15], g = ce[f6 - 2], y4 = J2(d2, 7) ^ J2(d2, 18) ^ d2 >>> 3, h4 = J2(g, 17) ^ J2(g, 19) ^ g >>> 10;
      ce[f6] = h4 + ce[f6 - 7] + y4 + ce[f6 - 16] | 0;
    }
    let { A: r2, B: o2, C: s2, D: i4, E: c3, F: u3, G: a2, H: l3 } = this;
    for (let f6 = 0; f6 < 64; f6++) {
      const d2 = J2(c3, 6) ^ J2(c3, 11) ^ J2(c3, 25), g = l3 + d2 + Bs(c3, u3, a2) + Cs[f6] + ce[f6] | 0, h4 = (J2(r2, 2) ^ J2(r2, 13) ^ J2(r2, 22)) + Ls(r2, o2, s2) | 0;
      l3 = a2, a2 = u3, u3 = c3, c3 = i4 + g | 0, i4 = s2, s2 = o2, o2 = r2, r2 = g + h4 | 0;
    }
...
function zn(e2) {
  if (!wt(e2)) throw new Error("Uint8Array expected");
}
var Ds = Array.from({ length: 256 }, (e2, t2) => t2.toString(16).padStart(2, "0"));
function Ms(e2) {
  zn(e2);
  let t2 = "";
  for (let n2 = 0; n2 < e2.length; n2++) t2 += Ds[e2[n2]];
  return t2;
}
function Vs(e2) {
...
function Yn(e2) {
  ...
  for (let o2 = 0, s2 = 0; o2 < n2; o2++, s2 += 2) {
    const i4 = Jn(e2.charCodeAt(s2)), c3 = Jn(e2.charCodeAt(s2 + 1));
...
function Zn(e2, t2, n2) {
  ...
  else if (wt(t2)) r2 = Uint8Array.from(t2);
...
function Qn(e2, t2, n2, r2) {
  if (!Fs(t2, n2, r2)) throw new Error("expected valid " + e2 + ": " + n2 + " <= n < " + r2 + ", got " + t2);
}
...
function Ws(e2, t2, n2) {
  if (t2 < ve) throw new Error("invalid exponent, negatives unsupported");
  if (n2 <= ve) throw new Error("invalid modulus");
  if (n2 === Ke) return ve;
  let r2 = Ke;
  for (; t2 > ve; ) t2 & Ke && (r2 = r2 * e2 % n2), e2 = e2 * e2 % n2, t2 >>= Ke;
  return r2;
}
function z(e2, t2, n2) {
  let r2 = e2;
  for (; t2-- > ve; ) r2 *= r2, r2 %= n2;
  return r2;
}
...
function Js(e2) {
  const t2 = zs(e2), { P: n2 } = t2, r2 = (b2) => er(b2, n2), o2 = t2.montgomeryBits, s2 = Math.ceil(o2 / 8), i4 = t2.nByteLength, c3 = t2.adjustScalarBytes || ((b2) => b2), u3 = t2.powPminus2 || ((b2) => Ws(b2, n2 - BigInt(2), n2));
  ...
  function f6(b2, _2) {
    ...
    for (let A2 = BigInt(o2 - 1); A2 >= xe; A2--) {
      ...
      j = r2(X * X), v2 = r2(k3 * r2(Z2 * Z2)), E2 = r2(S2 * $), B2 = r2(p2 * (S2 + r2(l3 * p2)));
    }
  ...
  function g(b2) {
    const _2 = Zn("u coordinate", b2, s2);
    return i4 === 32 && (_2[31] &= 127), Xn(_2);
  }
  function y4(b2) {
  ...
  function h4(b2, _2) {
    const O3 = g(_2), k3 = y4(b2), E2 = f6(O3, k3);
  ...
  return { scalarMult: h4, scalarMultBase: L, getSharedSecret: (b2, _2) => h4(b2, _2), getPublicKey: (b2) => L(b2), utils: { randomPrivateKey: () => t2.randomBytes(t2.nByteLength) }, GuBytes: m2 };
}
...
BigInt(8);
function Qs(e2) {
  const t2 = BigInt(10), n2 = BigInt(20), r2 = BigInt(40), o2 = BigInt(80), s2 = xt, c3 = e2 * e2 % s2 * e2 % s2, u3 = z(c3, tr, s2) * c3 % s2, a2 = z(u3, Ys, s2) * e2 % s2, l3 = z(a2, Zs, s2) * a2 % s2, f6 = z(l3, t2, s2) * l3 % s2, d2 = z(f6, n2, s2) * f6 % s2, g = z(d2, r2, s2) * d2 % s2, y4 = z(g, o2, s2) * g % s2, h4 = z(y4, o2, s2) * g % s2, m2 = z(h4, t2, s2) * l3 % s2;
  return { pow_p_5_8: z(m2, tr, s2) * e2 % s2, b2: c3 };
}
function ei(e2) {
...
function sr(r2, e2) {
  ...
  var a2 = r2.length, c3 = r2.charAt(0), h4 = Math.log(a2) / Math.log(256), u3 = Math.log(256) / Math.log(a2);
  function d2(l3) {
    ...
    for (var b2 = 0, x3 = 0, I2 = 0, D = l3.length; I2 !== D && l3[I2] === 0; ) I2++, b2++;
    for (var j = (D - I2) * u3 + 1 >>> 0, T2 = new Uint8Array(j); I2 !== D; ) {
      for (var q2 = l3[I2], J3 = 0, K2 = j - 1; (q2 !== 0 || J3 < x3) && K2 !== -1; K2--, J3++) q2 += 256 * T2[K2] >>> 0, T2[K2] = q2 % a2 >>> 0, q2 = q2 / a2 >>> 0;
      if (q2 !== 0) throw new Error("Non-zero carry");
      x3 = J3, I2++;
    }
    for (var H = j - x3; H !== j && T2[H] === 0; ) H++;
    for (var me2 = c3.repeat(b2); H < j; ++H) me2 += r2.charAt(T2[H]);
    return me2;
  }
  function g(l3) {
    ...
    var b2 = 0;
    if (l3[b2] !== " ") {
      for (var x3 = 0, I2 = 0; l3[b2] === c3; ) x3++, b2++;
      for (var D = (l3.length - b2) * h4 + 1 >>> 0, j = new Uint8Array(D); l3[b2]; ) {
        var T2 = t2[l3.charCodeAt(b2)];
        if (T2 === 255) return;
...
var dr = (r2, e2, t2, s2) => {
  ...
  for (let u3 = 0; u3 < n2; ++u3) {
    const d2 = i4[r2[u3]];
    if (d2 === void 0) throw new SyntaxError(`Non-${s2} character`);
...
var gr = (r2, e2, t2) => {
  ...
  let n2 = "", o2 = 0, a2 = 0;
  for (let c3 = 0; c3 < r2.length; ++c3) for (a2 = a2 << 8 | r2[c3], o2 += 8; o2 > t2; ) o2 -= t2, n2 += e2[i4 & a2 >> o2];
  if (o2 && (n2 += e2[i4 & a2 << t2 - o2]), s2) for (; n2.length * t2 & 7; ) n2 += "=";
  return n2;
};
...
function ai2(r2, e2, t2) {
  e2 = e2 || [], t2 = t2 || 0;
  for (var s2 = t2; r2 >= tn; ) e2[t2++] = r2 & 255 | oi2, r2 /= 128;
  for (; r2 & en; ) e2[t2++] = r2 & 255 | oi2, r2 >>>= 7;
...
*/
//# sourceMappingURL=chunk-6IZTWMEN.js.map
